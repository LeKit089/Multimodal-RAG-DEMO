...
class MDA(nn.Module):
    def __init__(self, input_num):
        super().__init__()
        # 相当于是block
        self.proj_1 = nn.Conv2d(in_channels=input_num, out_channels=256, kernel_size=1)
        self.ConvGN = nn.GroupNorm(num_groups=32, num_channels=256)

        # 开始相当于att
        self.proj_3 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=1)
        self.gelu = nn.GELU()
        self.da = LKA(256) # 两个一起定义
    ...
    def forward(self, x):
        ...
        x = x + self.drop( self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * att)
...
class AC_FPN_DCM(Backbone):
    _fuse_type: torch.jit.Final[str]

    def __init__(
            self, bottom_up, in_features, out_channels, norm="", top_block=None, fuse_type="sum"
    ):
        super(AC_FPN_DCM, self).__init__()
        ...
        # print('---in_channels_per_feature: ', in_channels_per_feature) # [768, 768, 768, 768]
    ...
    def forward(self, x):
        ...
        # last_inner_t = self.conv_1x1_6(x_t[-1])
